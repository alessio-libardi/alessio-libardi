...
export class AppComponent implements OnInit {
  ...
  wheelDirection$ = this.wheel$.pipe(
    map((wheel) => (wheel.deltaY < 0 ? 'Up' : 'Down'))
  );

  pointerDown$ = fromEvent<PointerEvent>(this.document, 'pointerdown');
  pointerMove$ = fromEvent<PointerEvent>(this.document, 'pointermove').pipe(
    debounceTime(250)
  );
  pointerDirection$ = zip(this.pointerDown$, this.pointerMove$).pipe(
    map(([pointerDown, pointerMove]) => {
      return pointerDown.y < pointerMove.y ? 'Up' : 'Down';
    })
  );

  scrollDirection$: Observable<'Up' | 'Down'> = merge(
    this.wheelDirection$,
